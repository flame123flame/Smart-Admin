...
export class SystemConstantComponent implements OnInit {
  ...
  initDataTable = () => {
      ...
      columns: [
        {
          ...
          data: 'constantKey', className: 'text-left'
        }, {
          data: 'constantValue'
        }, {
          render: (data, type, full, meta) => {
            ...
            _btn += `<button type="button" class="btn btn-danger btn-social-icon" id="delete"><i class="fa fa-trash" aria-hidden="true"></i></button>`;
            return _btn;
          },
          className: "text-center"
        }
    ...
    this.dataTable.on('click', 'tbody tr button#delete', (e) => {
      var closestRow = $(e.target).closest('tr');
      var data = this.dataTable.row(closestRow).data();
  ...
  onClickDelete(){
    this.commonService.loading();
    this.ajax.doPost(URL.DELETE, { constantId : this.constantId }).subscribe((res: ResponseData<any>) => {
     if (MessageService.MSG.SUCCESS == res.status) {
      this.modalSuccess.openModal()
       this.getList();
     }else{
